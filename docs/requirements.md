...
94. `NeuronGraphRAG.search()` は引用外の明示的な除外 marker から肯定検索節と0個以上の除外節を分離し、肯定節だけを entry retrieval と graph propagation に渡す。除外判定は gold、expected、forbidden、評価 label を受け取らず、final rank 確定後、precision control がある場合は accepted 全候補の後、`limit` の前に一度だけ適用する。語の部分一致、引用内 marker、独立した曖昧な否定、文書側が除外対象を明示的に否定する記述を除外一致と扱わない。除外なしは従来の query、順位、hit explanation、diagnostics を変更しない。除外された hit の理由は trace diagnostics、通過 hit の判定は `SearchHit.explain()` に保持し、小規模 fixture の回帰は一般的な自然言語理解や統合検索品質を証明しない。詳細は [Production exclusion intent](production-exclusion-intent.md) で定義する。
