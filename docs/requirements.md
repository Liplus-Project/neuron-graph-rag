...
94. opt-in CPU shortlist retrieval は pinned multilingual-e5-small の structural chunk embedding と document centroid を revision / content hash 付き SQLite cache へ事前計算し、追加・変更・削除文書だけを差分更新する。query は E5 centroid 全件順位の K=50、各候補内 E5 cosine 上位2 chunk、pinned bge-reranker-v2-m3、NLME tau=1、source ID tie-break を固定する。既存 sparse / dense / graph score と尺度を混ぜず、既定 `search` と #230 semantic opt-in を変更しない独立 API だけに接続する。cache不一致、未構築、timeout、cancel、model identity不一致は fallbackせず明示的に失敗し、進捗と E5 / v2-m3 runtime、forward pair数、peak RSS を返す。model取得は明示操作に限り、import時に重い依存を読み込まない。独立 source-grounded query群と CPU 4 thread warm-cache query 60秒以内を公開条件とし、未達なら利用導線を公開しない。#244 development queryは実装一致の回帰にだけ使い、registered v5 gold / holdout と #234〜#244の凍結証拠を変更しない。
