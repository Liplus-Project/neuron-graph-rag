...
その後、mainから作成したfresh Windows worktreeでは、v2 judge prompt、development packet、judge artifactもLFの固定hashに対してCRLFでcheckoutされ、同じ内容のままmanifest読込とobserved artifact auditが停止した。verificationをv1だけに限定すると同じfrozen text契約をv2へ一貫して適用できないため、v1 は manifest が明示する baseline commit、v2 は manifest 登録 commit の blob bytes を取得し、frozen text-byte verificationを次の順序へ揃えた。current working tree の同名 path は historical evidence に使わない。
