...
| [github-rag-mcp-replacement-compatibility](https://github.com/Liplus-Project/neuron-graph-rag/wiki/github-rag-mcp-replacement-compatibility) | active | public GitHub repository一つのread-only snapshotをNGR local indexへ接続する。github-rag-mcp `search_issues` の保存済み capture がある場合だけ source URL と根拠を比較し、capture がない結果は adapter pipeline validation の判定不能として保存する。production github-rag-mcp、MCP authentication / transport、remote deployment、default変更は含まない。 |
