...
$volume = "github-structural-length-bias-ablation-v1-runtime"
$containerRoot = "/opt/ngr-structural-ablation-v1"
...
if ($Action -in @("audit", "probe")) {
    ...
    Invoke-Checked -Command @(
        $python, "-m", "neuron_graph_rag.structural_representation_length_bias_ablation", $Action,
        "--root", $root
    )
    exit 0
}
...
if ($Action -eq "preflight") {
    ...
    $sourceMap = [ordered]@{
        ...
        "package-init-fixture" = @("tests\fixtures\full_corpus_rerank_oracle_v2.package_init.py", "tests/fixtures/full_corpus_rerank_oracle_v2.package_init.py")
        ...
        "manifest" = @("tests\fixtures\structural_representation_length_bias_ablation_v1.manifest.json", "tests/fixtures/structural_representation_length_bias_ablation_v1.manifest.json")
        "query" = @("tests\fixtures\full_corpus_rerank_oracle_v2.query.json", "tests/fixtures/full_corpus_rerank_oracle_v2.query.json")
        "schema" = @("tests\fixtures\structural_representation_length_bias_ablation_v1.schema.json", "tests/fixtures/structural_representation_length_bias_ablation_v1.schema.json")
    ...
    Invoke-Checked -Command (@("wslc") + $arguments + @(
        "-m", "neuron_graph_rag.structural_representation_length_bias_ablation", "preflight",
...
try {
    Invoke-Checked -Command (@("wslc") + $arguments + @(
        "-m", "neuron_graph_rag.structural_representation_length_bias_ablation", "claim",
        "--root", $containerSource, "--runtime-root", $containerRun,
        "--source-commit", $sourceCommit
    ))
    foreach ($kind in @("base", "v2-m3")) {
        Invoke-Checked -Command (@("wslc") + $arguments + @(
            "-m", "neuron_graph_rag.structural_representation_length_bias_ablation", "worker",
            "--root", $containerSource, "--cache", $containerCache,
    ...
    Invoke-Checked -Command (@("wslc") + $finalizeArguments + @(
        "-m", "neuron_graph_rag.structural_representation_length_bias_ablation", "finalize",
        "--root", $containerSource, "--runtime-root", $containerRun,
        "--source-commit", $sourceCommit
    ))
}
    ...
    try {
        Invoke-Checked -Command (@("wslc") + $arguments + @(
            "-m", "neuron_graph_rag.structural_representation_length_bias_ablation", "record-error",
    ...
    $export = @(
        ...
        "set -eu; mkdir -p '/output/tests/evidence/structural_representation_length_bias_ablation_v1'; " +
        "if test -f '${containerRun}/preflight.attestation.json'; then " +
        "test ! -e '/output/tests/evidence/structural_representation_length_bias_ablation_v1/development.preflight.json'; " +
        "cp '${containerRun}/preflight.attestation.json' " +
        "'/output/tests/evidence/structural_representation_length_bias_ablation_v1/development.preflight.json'; fi; " +
        "if test -f '${containerSource}/tests/evidence/structural_representation_length_bias_ablation_v1/development.claim.json'; then " +
        "test ! -e '/output/tests/evidence/structural_representation_length_bias_ablation_v1/development.claim.json'; " +
        "cp '${containerSource}/tests/evidence/structural_representation_length_bias_ablation_v1/development.claim.json' " +
        "'/output/tests/evidence/structural_representation_length_bias_ablation_v1/development.claim.json'; fi; " +
        "if test -f '${containerSource}/tests/evidence/structural_representation_length_bias_ablation_v1/development.observed.json'; then " +
        "test ! -e '/output/tests/evidence/structural_representation_length_bias_ablation_v1/development.observed.json'; " +
        "cp '${containerSource}/tests/evidence/structural_representation_length_bias_ablation_v1/development.observed.json' " +
        "'/output/tests/evidence/structural_representation_length_bias_ablation_v1/development.observed.json'; fi; " +
        "if test -f '${containerSource}/tests/evidence/structural_representation_length_bias_ablation_v1/development.error.json'; then " +
        "test ! -e '/output/tests/evidence/structural_representation_length_bias_ablation_v1/development.error.json'; " +
        "cp '${containerSource}/tests/evidence/structural_representation_length_bias_ablation_v1/development.error.json' " +
        "'/output/tests/evidence/structural_representation_length_bias_ablation_v1/development.error.json'; fi"
    )
