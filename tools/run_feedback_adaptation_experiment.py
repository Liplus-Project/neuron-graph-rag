...
from neuron_graph_rag.feedback_adaptation_experiment import (
    run_feedback_adaptation_development,
    run_feedback_adaptation_holdout,
    write_feedback_adaptation_result,
)


def main() -> int:
    parser = argparse.ArgumentParser(description="Run one frozen feedback adaptation stage.")
    ...
    if args.stage == "development":
        ...
        result = run_feedback_adaptation_development(args.manifest)
    else:
        if args.development_result is None:
            raise SystemExit("holdout requires --development-result")
        result = run_feedback_adaptation_holdout(args.manifest, args.development_result)
    write_feedback_adaptation_result(args.output, result)
    return 0
