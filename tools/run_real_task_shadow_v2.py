...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the result-free real-task shadow v2 protocol")
    commands = parser.add_subparsers(dest="command", required=True)
    probe = commands.add_parser("probe", help="run the unregistered runtime-shaped placeholder round-trip")
    probe.add_argument("--fixture", type=Path, required=True)
    capture = commands.add_parser("capture", help="append one v2 packet to a local registry")
    capture.add_argument("--input", type=Path, required=True)
    capture.add_argument("--registry-dir", type=Path, required=True)
    verify_packet = commands.add_parser("verify-packet", help="verify a v2 packet against its snapshot")
    verify_packet.add_argument("--packet", type=Path, required=True)
    verify_packet.add_argument("--snapshot", type=Path, required=True)
    replay = commands.add_parser("replay", help="cumulatively replay v2 packets into both shadow arms")
    ...
    verify_result = commands.add_parser("verify-result", help="recompute and verify an exact v2 result")
    ...
    if args.command == "probe":
        ...
        result = replay_packet(read_canonical_json(args.packet), args.snapshot) if args.packet else replay_registry(args.registry_dir, args.snapshot)
        write_json_exclusive(args.output, result)
        print(args.output)
    else:
        result = read_canonical_json(args.result)
        if args.packet:
            verify_result_against_packets(result, [read_canonical_json(args.packet)], args.snapshot)
        else:
