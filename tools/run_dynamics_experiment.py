...
from neuron_graph_rag.experiment import run_development, run_holdout, write_result


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run a frozen neural dynamics experiment stage."
    )
    ...
    if args.stage == "development":
        ...
        result = run_development(args.manifest)
    else:
        if args.development_result is None:
            raise SystemExit("holdout requires --development-result")
        result = run_holdout(args.manifest, args.development_result)
