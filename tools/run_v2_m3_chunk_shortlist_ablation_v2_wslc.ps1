...
$experimentRoot = Join-Path $workspace "workspace\experiments\v2_m3_chunk_shortlist_ablation_v2"
$buildContext = Join-Path $experimentRoot "bundle-image-context"
$ceHostCache = Join-Path $workspace "workspace\experiments\practical_two_stage_retrieval_v1\model-cache"
$e5HostCache = Join-Path $workspace ".semantic230-model"
$volume = "github-v2-m3-chunk-shortlist-ablation-v2-runtime"
$containerRoot = "/opt/ngr-v2-m3-chunk-shortlist-v2"
...
$module = "neuron_graph_rag.v2_m3_chunk_shortlist_ablation_v2"
...
if ($Action -eq "preflight") {
    ...
    $sourceMap = [ordered]@{
        ...
        "src\neuron_graph_rag\v2_m3_chunk_shortlist_ablation_v2.py" = "src/neuron_graph_rag/v2_m3_chunk_shortlist_ablation_v2.py"
        ...
        "tests\fixtures\v2_m3_chunk_shortlist_ablation_v2.manifest.json" = "tests/fixtures/v2_m3_chunk_shortlist_ablation_v2.manifest.json"
        "tests\fixtures\v2_m3_chunk_shortlist_ablation_v2.models.json" = "tests/fixtures/v2_m3_chunk_shortlist_ablation_v2.models.json"
        "tests\fixtures\v2_m3_chunk_shortlist_ablation_v2.schema.json" = "tests/fixtures/v2_m3_chunk_shortlist_ablation_v2.schema.json"
        "tools\run_v2_m3_chunk_shortlist_ablation_v2_wslc.ps1" = "tools/run_v2_m3_chunk_shortlist_ablation_v2_wslc.ps1"
    }
    ...
    $ceSnapshot = "models--BAAI--bge-reranker-v2-m3\snapshots\953dc6f6f85a1b2dbfca4c34a2796e7dde08d41e"
    Get-ChildItem -LiteralPath (Join-Path $ceHostCache $ceSnapshot) -File | ForEach-Object {
        $to = Join-Path $resolvedContext "ce-cache\$ceSnapshot\$($_.Name)"
        New-Item -ItemType Directory -Force (Split-Path -Parent $to) | Out-Null
    ...
    Invoke-Checked -Command (@("wslc") + (Base-Arguments -Image $ceImage -Memory "8G") + @("-m", $module, "preflight-v2-m3", "--root", $containerSource, "--cache", $containerCECache, "--runtime-root", $containerRun, "--source-commit", $sourceCommit))
...
try {
    ...
    Invoke-Checked -Command (@("wslc") + $ceArguments + @("-m", $module, "stage2", "--root", $containerSource, "--cache", $containerCECache, "--runtime-root", $containerRun))
    $goldDirectory = Join-Path $root "tests\fixtures"
    $copyGold = "tar -cf - -C $goldDirectory full_corpus_rerank_oracle_v2.gold.json | wslc run --rm --interactive --network none --volume ${volume}:${containerRoot} --entrypoint /bin/tar $ceImage -xf - -C ${containerSource}/tests/fixtures"
    ...
    $evidenceRoot = Join-Path $root "tests\evidence\v2_m3_chunk_shortlist_ablation_v2"
    $exports = [ordered]@{
        "${containerRun}/preflight.attestation.json" = (Join-Path $evidenceRoot "development.preflight.json")
        "${containerSource}/tests/evidence/v2_m3_chunk_shortlist_ablation_v2/development.claim.json" = (Join-Path $evidenceRoot "development.claim.json")
        "${containerRun}/stage1.worker.json" = (Join-Path $evidenceRoot "development.stage1.worker.json")
        "${containerRun}/stage2.worker.json" = (Join-Path $evidenceRoot "development.stage2.worker.json")
        "${containerSource}/tests/evidence/v2_m3_chunk_shortlist_ablation_v2/development.observed.json" = (Join-Path $evidenceRoot "development.observed.json")
        "${containerSource}/tests/evidence/v2_m3_chunk_shortlist_ablation_v2/development.error.json" = (Join-Path $evidenceRoot "development.error.json")
    }
