...
def _identifiers(fixture: Path, gold: Path | None = None) -> dict[str, set[str]]:
    ...
    identifiers = {
        ...
        "edge_identities": {
            "\u0000".join((str(edge["source_id"]), str(edge["target_id"]), str(edge["edge_type"])))
    ...
    if gold is not None:
        ...
        identifiers["source_urls"] = {str(case["source_url"]) for case in cases}
        identifiers["queries"] = {str(case["query"]).strip().lower() for case in cases}
        identifiers["queries"].add(str(schedule["feedback"]["query"]).strip().lower())
    return identifiers


def main() -> int:
    parser = argparse.ArgumentParser(description="Audit a disjoint feedback adaptation reproduction fixture.")
    ...
    result = {
        ...
        "inputs": {
            "development_fixture_sha256": _sha(args.development_fixture),
            "development_gold_sha256": _sha(args.development_gold),
            "holdout_fixture_sha256": _sha(args.holdout_fixture),
            "holdout_gold_sha256": _sha(args.holdout_gold),
