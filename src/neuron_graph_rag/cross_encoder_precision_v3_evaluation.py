...
STEM = "github_cross_encoder_precision_v3"
PROTOCOL_ID = "github-ngr-cross-encoder-precision-v3"
...
GATE_IDS = (
    ...
    "cross-encoder-fusion-rank-only-recomputation",
...
def validate_protocol(protocol: Mapping[str, Any]) -> None:
    ...
    _verify_hash_registry(root, _mapping(manifest, "v2_immutable_sha256"))
    ...
    if set(_mapping(protocol, "candidates")) != {
        "protocol_id",
        "rrf_constant",
        "top_k",
        "selection",
        ...
        raise ValueError("rank-only candidate contract cannot contain a threshold")
    candidate_contract = _mapping(protocol, "candidates")
    if (
        candidate_contract.get("rrf_constant") != 60
        or candidate_contract.get("top_k") != 5
        or candidate_contract.get("selection")
        != "first all-hard-gates-pass candidate in frozen order; otherwise holdout remains closed"
    ):
        raise ValueError("rank-only fusion constants mismatch")
    expected_candidates = (
        (CANDIDATE_IDS[0], "BAAI/bge-reranker-base", "rrf"),
        (CANDIDATE_IDS[1], "BAAI/bge-reranker-base", "ce"),
        (CANDIDATE_IDS[2], "BAAI/bge-reranker-v2-m3", "rrf"),
        (CANDIDATE_IDS[3], "BAAI/bge-reranker-v2-m3", "ce"),
    )
    for row, expected in zip(candidates, expected_candidates, strict=True):
        if (
            set(row) != {"candidate_id", "model_id", "fusion"}
            or (row["candidate_id"], row["model_id"], row["fusion"]) != expected
        ):
            raise ValueError("rank-only candidate shape mismatch")
...
def _derive_candidate(
    ...
    for model_case in _object_list(model, "cases"):
        prefilter_hits = model_case["ranked_hits"]
        ce_sorted = sorted(
            prefilter_hits,
...
def main(argv: list[str] | None = None) -> int:
    ...
    output: dict[str, Any] = {
        ...
        "historical_v1_v2_observations_included": False,
