...
from .cross_encoder_precision_evaluation import (
    PROTOCOL_ID,
    ROOT,
    archive_stage,
...
PROTOCOL_COMMIT = "b681b2a8073e13dfc940b6f3e0c55f86556a21c0"
SOURCE_COMMIT = "c32b3049fd3daaa2190faf5e3e85955a195ee88c"
OBSERVATION_NOW = 1_700_000_000.0
BATCH_SIZE = 8
EVIDENCE = Path("tests/evidence/github_cross_encoder_precision_v1")
PREFLIGHT_FILES = (
    "preflight.json",
...
def default_external_root() -> Path:
    return ROOT.parent.parent / "experiments" / "github_cross_encoder_precision_v1"
...
def preflight(root: Path = ROOT, external_root: Path | None = None) -> dict[str, Any]:
    ...
    lock = root / "tests/fixtures/github_cross_encoder_precision_v1.requirements.lock"
    _run_logged(
    ...
    online = _worker_environment(root, offline=False)
    fetch_report_path = external / "model-verification.json"
    ...
    _run_logged(
        [
            str(python),
            "-m",
            "neuron_graph_rag.cross_encoder_precision_observation",
            "model-fetch",
            "--cache",
            str(cache),
            "--output",
    ...
    model_report = read_json(fetch_report_path)
    ...
    for arguments in (
        [str(python), "-m", "unittest", "tests.test_cross_encoder_precision"],
        [
            str(python),
            "-m",
            "unittest",
            "tests.test_cross_encoder_precision_observation",
        ],
        [
            str(python),
            "-m",
            "neuron_graph_rag.cross_encoder_precision_evaluation",
            "audit",
        ],
        [
            str(python),
    ...
    verify_preflight(root, external)
    return preflight_report


def verify_preflight(
    root: Path = ROOT, external_root: Path | None = None
) -> dict[str, Any]:
    external = (external_root or default_external_root()).resolve()
    protocol = load_protocol(root)
    verify_protocol_commit(PROTOCOL_COMMIT, protocol)
...
def run_conditional(
    ...
    external = (external_root or default_external_root()).resolve()
    preflight_report = verify_preflight(root, external)
    before = preflight_report["shared_database_sha256_before"]
    execution_rows: list[dict[str, Any]] = []
    development: dict[str, Any] | None = None
    holdout: dict[str, Any] | None = None
    try:
        development = _run_stage_once("development", root, external, execution_rows)
        if development.get("all_hard_gates_pass") is True:
            holdout = _run_stage_once("holdout", root, external, execution_rows)
...
def _run_stage_once(
    ...
    try:
        baseline_primary = _run_worker(
            root, external, stage, "baseline", "primary", stage_root, command_rows
        )
        baseline_replay = _run_worker(
            root, external, stage, "baseline", "replay", stage_root, command_rows
        )
        baseline = {
        ...
        for model_key in ("base", "v2-m3"):
            primary = _run_worker(
                root, external, stage, model_key, "primary", stage_root, command_rows
            )
            replay = _run_worker(
                root, external, stage, model_key, "replay", stage_root, command_rows
            )
...
def model_fetch(cache: Path, output: Path) -> None:
    from huggingface_hub import snapshot_download

    protocol = load_protocol()
    rows = []
    for spec in _rows(_mapping(protocol, "models"), "models"):
        required = [_string(row, "path") for row in _rows(spec, "required_files")]
        snapshot = Path(
            snapshot_download(
                repo_id=_string(spec, "model_id"),
                revision=_string(spec, "revision"),
                cache_dir=cache,
...
def main(argv: list[str] | None = None) -> int:
    ...
    fetch = commands.add_parser("model-fetch")
    ...
    if arguments.command == "preflight":
        result = preflight(ROOT, arguments.external_root)
    elif arguments.command == "verify-preflight":
        result = verify_preflight(ROOT, arguments.external_root)
    elif arguments.command == "run":
        result = run_conditional(ROOT, arguments.external_root)
    elif arguments.command == "model-fetch":
        model_fetch(arguments.cache, arguments.output)
