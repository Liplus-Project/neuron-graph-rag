"""Result-free retrieval parity v5 protocol and append-only lifecycle audit."""
...
STEM = "github_retrieval_parity_v5"
PROTOCOL_ID = "github-rag-vs-ngr-retrieval-parity-v5"
SOURCE_REPOSITORY = "Liplus-Project/neuron-graph-rag"
SOURCE_REF = "main"
...
PREFLIGHT_DOCUMENT_FIELDS = (
    "repo",
    "type",
    "doc_path",
    "stored_content_prefix",
...
CORPUS_PATH = FIXTURES / "github_retrieval_parity_v4.corpus.json"
QUERIES_PATH = FIXTURES / f"{STEM}.queries.json"
GOLD_PATH = FIXTURES / f"{STEM}.gold.json"
GATE_PATH = FIXTURES / f"{STEM}.gate.json"
PREFLIGHT_SCHEMA_PATH = FIXTURES / f"{STEM}.preflight-schema.json"
...
def load_protocol(
    ...
    relatives = {
        ...
        "preflight_schema": PREFLIGHT_SCHEMA_PATH.relative_to(ROOT),
    ...
    protocol: dict[str, Any] = {
        ...
        "preflight_schema": read_json(paths["preflight_schema"]),
        ...
        "corpus_mapping": read_json(paths["corpus"]),
    }
...
def validate_protocol(protocol: Mapping[str, Any]) -> None:
    ...
    for artifact in (
        ...
        gold,
        gate,
        _mapping(protocol, "preflight_schema"),
    ...
    if manifest.get("issue") != 226 or manifest.get("phase") != "result-free-freeze-v5":
    ...
    expected_protocol_artifacts = {
        ...
        for name, path in {
            ...
            "gold": GOLD_PATH,
            "gate": GATE_PATH,
            "preflight_schema": PREFLIGHT_SCHEMA_PATH,
    ...
    if dict(_mapping(manifest, "lifecycle_contract")) != {
        "preflight_policy": "exclusive-create-before-registered-query",
        ...
        "freeze_identity_scope": (
            "referenced-v4-corpus-protocol-artifacts-and-production-runtime"
        ),
        "observation_identity_scope": "registered-preflight-capture-claim-result",
    }:
        raise ValueError("lifecycle contract mismatch")

    preflight_schema = _mapping(protocol, "preflight_schema")
    ...
    if (
        tuple(preflight_schema.get("top_level_fields", ())) != PREFLIGHT_FIELDS
        or tuple(preflight_schema.get("document_fields", ()))
        != PREFLIGHT_DOCUMENT_FIELDS
        or tuple(capture_schema.get("top_level_fields", ())) != CAPTURE_FIELDS
        ...
        raise ValueError("preflight, capture, claim, or result schema mismatch")
    ...
    if (
        ...
        or source.get("path_prefix") != PATH_PREFIX
        or source.get("path_glob") != f"{PATH_PREFIX}**/*.md"
        or source.get("document_count") != 93
        or source.get("access")
        != "path-prefix-isolated-github-index-and-public-source-read"
        or source.get("corpus_fixture")
        != "tests/fixtures/github_retrieval_parity_v4.corpus.json"
        or source.get("reused_from_protocol")
        != "github-rag-vs-ngr-retrieval-parity-v4"
        or not PATH_PREFIX.endswith("/")
        or len(PATH_PREFIX.encode("utf-8")) > 64
    ):
        raise ValueError("path-isolated source contract mismatch")
    if (root / "tests/fixtures/github_retrieval_parity_v5.corpus.json").exists():
        raise ValueError("v5 must reference the v4 corpus instead of duplicating it")
    materialized_from = _mapping(source, "materialized_from")
    v3_corpus_path = root / "tests/fixtures/github_retrieval_parity_v3.corpus.json"
    if (
        materialized_from.get("repository") != "Liplus-Project/liplus-language"
        or materialized_from.get("commit")
        != "51623e200ab6128cf59cf5b5ac7d8115c31268d6"
        or materialized_from.get("tree_sha")
        != "034101e7ff123f91d089640e0685514a3a890159"
        or materialized_from.get("fixture")
        != "tests/fixtures/github_retrieval_parity_v3.corpus.json"
        or materialized_from.get("fixture_sha256") != _sha256(v3_corpus_path)
    ):
        raise ValueError("v3 corpus derivation provenance mismatch")

    paths = source.get("paths")
    corpus_mapping = _mapping(protocol, "corpus_mapping")
    corpus_documents = corpus_mapping.get("documents")
    if (
        corpus.repository != SOURCE_REPOSITORY
        or corpus.commit != SOURCE_REF
        or corpus_mapping.get("path_prefix") != PATH_PREFIX
        or corpus_mapping.get("corpus_identity_sha256")
        != source.get("corpus_identity_sha256")
        or not isinstance(paths, list)
        or len(paths) != 93
        or not isinstance(corpus_documents, list)
        or len(corpus_documents) != 93
        or sorted(paths) != [document.path for document in corpus.documents]
        or any(
            not isinstance(item, str)
            or not item.startswith(PATH_PREFIX)
        ...
        raise ValueError("corpus does not match the frozen path-prefix surface")
    _validate_snapshot_hashes(corpus)

    v3_mapping = read_json(v3_corpus_path)
    v3_documents = {item["path"]: item for item in v3_mapping["documents"]}
    identity_rows: list[dict[str, str]] = []
    materialized_files = sorted(
        item.relative_to(root).as_posix()
        for item in (root / PATH_PREFIX).rglob("*")
        if item.is_file()
    )
    if materialized_files != sorted(paths):
        raise ValueError("prefix contains missing, duplicate, or out-of-corpus files")
    for item in corpus_documents:
        if not isinstance(item, Mapping):
            raise TypeError("corpus documents must be objects")
        path_value = _required_string(item, "path")
        original = _mapping(item, "materialized_from")
        original_path = _required_string(original, "path")
        source_row = v3_documents.get(original_path)
        if (
            path_value != f"{PATH_PREFIX}{original_path}"
            or source_row is None
            or original.get("repository") != v3_mapping["repository"]
            or original.get("commit") != v3_mapping["commit"]
            or original.get("source_url") != source_row["source_url"]
            or item.get("blob_sha") != source_row["blob_sha"]
            or item.get("content_sha256") != source_row["content_sha256"]
            or item.get("content") != source_row["content"]
            or (root / path_value).read_bytes() != source_row["content"].encode("utf-8")
        ):
            raise ValueError(f"materialized corpus drifted: {path_value}")
    ...
    if _json_sha256(identity_rows) != source.get("corpus_identity_sha256"):
        raise ValueError("materialized corpus identity mismatch")
    ...
    if defaults != {
        ...
        raise ValueError("request defaults are not the frozen path-prefix request")
    if _mapping(preflight_schema, "request_contract") != {
        "repo": SOURCE_REPOSITORY,
        "type": "doc",
        "path_prefix": PATH_PREFIX,
        "query_omitted": True,
        ...
        raise ValueError("preflight request contract mismatch")
    ...
    v4_protocol_parts = {
        name: read_json(
            root / f"tests/fixtures/github_retrieval_parity_v4.{suffix}.json"
        )
        for name, suffix in {
            "queries": "queries",
            "gold": "gold",
    ...
    for name in (
        ...
        current = dict(_mapping(protocol, name))
        predecessor = dict(v4_protocol_parts[name])
        current.pop("protocol_id", None)
        predecessor.pop("protocol_id", None)
        if current != predecessor:
            raise ValueError(f"v5 {name} semantics drifted from v4")
    v4_queries = v4_protocol_parts["queries"]
    if (
        dict(_mapping(queries, "request_defaults"))
        != dict(_mapping(v4_queries, "request_defaults"))
        or dict(_mapping(queries, "ngr_engine_config"))
        != dict(_mapping(v4_queries, "ngr_engine_config"))
    ):
        raise ValueError("v5 request or NGR contract drifted from v4")

    query_stages = _mapping(queries, "stages")
    gold_stages = _mapping(gold, "stages")
    v4_query_stages = _mapping(v4_queries, "stages")
    v4_gold_stages = _mapping(v4_protocol_parts["gold"], "stages")
    ...
    for stage in STAGES:
        ...
        v4_cases = _list_value(v4_query_stages, stage)
        v4_rows = _list_value(v4_gold_stages, stage)
        if len(cases) != len(COHORTS) or len(rows) != len(cases):
            raise ValueError("each stage must contain one case per cohort")
        if len(v4_cases) != len(cases) or len(v4_rows) != len(rows):
            raise ValueError("v5 query or gold cardinality drifted from v4")
        for current_case, v4_case, current_gold, v4_gold in zip(
            cases, v4_cases, rows, v4_rows, strict=True
        ):
            expected_case_id = str(v4_case["case_id"]).replace("v4path-", "v5path-")
            if current_case != {
                **v4_case,
                ...
                **v4_gold,
                ...
                raise ValueError("v5 query or gold semantics drifted from v4")
        gold_by_id = {
        ...
        for case in cases:
            ...
            query_text = _required_string(case, "query")
            query_hash = hashlib.sha256(query_text.encode("utf-8")).hexdigest()
            if query_hash in all_query_hashes:
                raise ValueError("query texts must be unique across stages")
            all_query_hashes.add(query_hash)
            ...
            expected = _source_id_list(row, "expected_source_ids", identities, required=True)
            ...
            if cohort == "over_exclusion_control" and (
                ...
                raise ValueError("over-exclusion control requires protected expected sources")
            if cohort == "over_exclusion_control":
                intent = decompose_exclusion_intent(query_text)
                if not intent.exclusion_clauses:
                    raise ValueError("over-exclusion control requires an explicit exclusion clause")
    ...
    predecessor_gold_sources: set[str] = set()
    predecessor_paths: set[str] = set()
    v4_query_hashes: set[str] | None = None
    predecessors = manifest.get("predecessors")
    expected_predecessors = {"retrieval-parity-v4"}
    ...
    v4_manifest = read_json(
        root / "tests/fixtures/github_retrieval_parity_v4.manifest.json"
    )
    expected_v4_identity_paths = {
        "tests/fixtures/github_retrieval_parity_v4.manifest.json",
        *_mapping(v4_manifest, "artifact_sha256"),
        *_mapping(v4_manifest, "runtime_sha256"),
    }
    expected_v4_observation_paths = {
        "tests/evidence/github_retrieval_parity_v4/development.preflight.error.json"
    }
    for inherited in _list_value(v4_manifest, "predecessors"):
        if not isinstance(inherited, Mapping):
            raise TypeError("v4 predecessor registry rows must be objects")
        expected_v4_identity_paths.update(_mapping(inherited, "identity_sha256"))
        expected_v4_observation_paths.update(
            _mapping(inherited, "observation_sha256")
        )
    for predecessor in predecessors:
        if not isinstance(predecessor, Mapping):
            raise TypeError("predecessor registry rows must be objects")
        if (
            set(_mapping(predecessor, "identity_sha256"))
            != expected_v4_identity_paths
            or set(_mapping(predecessor, "observation_sha256"))
            != expected_v4_observation_paths
        ):
            raise ValueError("v1-v4 predecessor hash registry is incomplete")
        for registry in ("identity_sha256", "observation_sha256"):
            for relative, expected_hash in _mapping(predecessor, registry).items():
        ...
        query_hashes = predecessor.get("query_text_sha256")
        gold_sources = predecessor.get("gold_source_ids")
        if (
            ...
            or not isinstance(gold_sources, list)
            or not all(isinstance(item, str) for item in gold_sources)
        ):
            raise ValueError("predecessor identity registry mismatch")
        predecessor_cases.update(case_ids)
        predecessor_gold_sources.update(gold_sources)
        if predecessor.get("name") == "retrieval-parity-v4":
            if predecessor.get("registered_query_execution_count") != 0:
                raise ValueError("v4 query derivation requires literal zero-query evidence")
            v4_audit = read_json(
                root / "tests/fixtures/github_retrieval_parity_v4.result-free-audit.json"
            )
            if (
                v4_audit.get("development_query_execution_count") != 0
                or v4_audit.get("holdout_query_execution_count") != 0
                or any(
                    v4_audit.get(key) != 0
                    for key in (
                        "development_preflight_count",
                        "development_capture_count",
                        "development_claim_count",
                        "development_result_count",
                        "holdout_preflight_count",
                        "holdout_capture_count",
                        "holdout_claim_count",
                        "holdout_result_count",
                    )
                )
            ):
                raise ValueError("v4 registered observation evidence is not result-free")
            failure = read_json(
                root
                / "tests/evidence/github_retrieval_parity_v4/development.preflight.error.json"
            )
            counts = _mapping(failure, "execution_counts")
            if (
                failure.get("terminal") is not True
                or failure.get("status") != "preflight-registration-failed"
                or _mapping(failure, "failure").get("attempt_count") != 1
                or _mapping(failure, "failure").get("retry_count") != 0
                or any(
                    counts.get(key) != 0
                    for key in (
                        "registered_preflight_count",
                        "registered_query_execution_count",
                        "registered_capture_count",
                        "registered_claim_count",
                        "registered_result_count",
                        "holdout_query_execution_count",
                        "shared_database_open_count",
                    )
                )
            ):
                raise ValueError("v4 terminal failure evidence is not zero-query")
            v4_query_hashes = set(query_hashes)
    if all_case_ids & predecessor_cases:
        raise ValueError("v5 case identity overlaps a predecessor")
    if v4_query_hashes is None or all_query_hashes != v4_query_hashes:
        raise ValueError("v5 query texts must derive exactly from zero-query v4")
    if set().union(*stage_identities.values()) != predecessor_gold_sources:
        raise ValueError("v5 gold source identity must match the v4 corpus contract")
    ...
    overlap = new_paths & predecessor_paths
    if overlap != set(RUNTIME_PATHS):
        raise ValueError("v5 artifact registry overlaps a predecessor")
    ...
    if gate_ids != GATE_IDS or not all(row.get("hard") is True for row in gate["gates"]):
        raise ValueError("hard gate order mismatch")
    expected_audit = {
        "protocol_id": PROTOCOL_ID,
        "phase": "result-free-freeze-v5",
        "development_preflight_count": 0,
        "development_query_execution_count": 0,
        "holdout_preflight_count": 0,
        ...
        "v4_query_text_derivation_count": 10,
    ...
    if dict(_mapping(manifest, "whole_module_probe")) != {
        "test_module": "tests.test_github_retrieval_parity_v5",
        "result_free_phase": "result-free",
        "synthetic_post_observation_phases": [
            "development-closed",
            "holdout-eligible",
        ],
        ...
        raise ValueError("whole-module or import-smoke contract mismatch")
    ...
    if require_result_free:
        for relative in _registry_paths(manifest):
            if (root / relative).exists():
                raise ValueError(
                    "registered preflight, capture, claim, and result must be absent at freeze"
                )
...
def register_preflight(
    ...
    _assert_preflight_can_start(stage, protocol)
    raw = source.read_bytes()
    value = json.loads(raw.decode("utf-8", errors="strict"))
    if not isinstance(value, Mapping):
        raise TypeError("preflight must be a JSON object")
    _validate_preflight(value, stage, protocol_commit, protocol)
    target = _stage_path(protocol, stage, "preflight")
...
def register_capture(
    ...
    preflight = read_json(_stage_path(protocol, stage, "preflight"))
    if preflight.get("protocol_commit") != protocol_commit:
        raise ValueError("preflight and registered capture commit mismatch")
    raw = source.read_bytes()
    value = json.loads(raw.decode("utf-8", errors="strict"))
    if not isinstance(value, Mapping):
...
def verify_protocol_commit(protocol_commit: str, protocol: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise ValueError("protocol_commit must first introduce the v5 manifest")
    ...
    if introductions.returncode != 0 or introductions.stdout.splitlines() != [
        ...
        raise ValueError("protocol_commit must uniquely introduce the v5 manifest")
    if (
...
def _run_ngr_once(
    ...
    with TemporaryDirectory(prefix="ngr-retrieval-parity-v5-") as directory:
...
def _validate_preflight(
    ...
    schema = _mapping(protocol, "preflight_schema")
    if list(preflight) != schema.get("top_level_fields"):
        raise ValueError("preflight top-level field order mismatch")
    if (
        preflight.get("protocol_id") != PROTOCOL_ID
        or preflight.get("stage") != stage
        or preflight.get("protocol_commit") != protocol_commit
        or preflight.get("service") != "github-rag-mcp+github-public-read"
        or preflight.get("tool") != "queryless-prefix-preflight"
    ):
        raise ValueError("preflight protocol identity mismatch")
    _required_string(preflight, "captured_at")
    if preflight.get("request") != schema.get("request_contract"):
        raise ValueError("preflight request differs from the frozen queryless request")
    corpus = protocol["corpus"]
    assert isinstance(corpus, GitHubSnapshot)
    corpus_documents = {document.path: document for document in corpus.documents}
    rows = preflight.get("documents")
    if not isinstance(rows, list) or len(rows) != len(corpus_documents):
        raise ValueError("preflight must contain the exact frozen document count")
    seen: set[str] = set()
    for row in rows:
        if not isinstance(row, Mapping) or list(row) != schema.get("document_fields"):
            raise ValueError("preflight document field order mismatch")
        path = _required_string(row, "doc_path")
        document = corpus_documents.get(path)
        if (
            document is None
            or path in seen
            or row.get("repo") != SOURCE_REPOSITORY
            or row.get("type") != "doc"
            or not path.startswith(PATH_PREFIX)
        ):
            raise ValueError("preflight found duplicate, missing, or out-of-prefix path")
        seen.add(path)
        if (
            row.get("stored_content_prefix")
            != _indexed_content(path, document.content)
            or row.get("source_content_sha256") != document.content_sha256
        ):
            raise ValueError("preflight stored content or source body hash drifted")
    if seen != set(corpus_documents):
        raise ValueError("preflight exact path set mismatch")


def verify_registered_preflight(stage: str, root: Path = ROOT) -> None:
    protocol = load_protocol(root, require_result_free=False)
    _validate_stage(stage)
    path = _stage_path(protocol, stage, "preflight")
    if not path.exists():
        raise FileNotFoundError("registered preflight is required")
    preflight = read_json(path)
    _validate_preflight(
        preflight,
        stage,
        _required_string(preflight, "protocol_commit"),
        protocol,
    )
...
def audit_repository_lifecycle(root: Path = ROOT) -> dict[str, Any]:
    ...
    if development["result"] is None:
        phase = (
            ...
            else "development-preflighted"
            if development["preflight"]
            else "result-free"
        )
    elif not eligible:
    ...
    elif holdout["preflight"]:
        phase = "holdout-preflighted"
    else:
    ...
    return {
        ...
        "phase": phase,
        "development": {
            "preflight": development["preflight"],
...
def _stage_registry(protocol: Mapping[str, Any], stage: str) -> dict[str, Any]:
    preflight_path = _stage_path(protocol, stage, "preflight")
    capture_path = _stage_path(protocol, stage, "capture")
    claim_path = _stage_path(protocol, stage, "claim")
    result_path = _stage_path(protocol, stage, "result")
    ...
    if capture and not preflight:
        raise ValueError("stage capture exists without preflight")
    ...
    if preflight:
        verify_registered_preflight(stage, Path(protocol["root"]))
...
def _assert_stage_can_start(stage: str, protocol: Mapping[str, Any]) -> None:
    preflight = _stage_path(protocol, stage, "preflight")
    if not preflight.exists():
        raise FileNotFoundError("registered query capture requires a passed preflight")
    verify_registered_preflight(stage, Path(protocol["root"]))
    if any(
...
def _assert_preflight_can_start(stage: str, protocol: Mapping[str, Any]) -> None:
    if any(
        _stage_path(protocol, stage, kind).exists()
        for kind in ("preflight", "capture", "claim", "result")
    ):
        raise FileExistsError(f"registered {stage} stage already exists")
    if stage == "holdout":
        development = _stage_path(protocol, "development", "result")
...
def _validate_snapshot_hashes(snapshot: GitHubSnapshot) -> None:
    if snapshot.commit != SOURCE_REF:
        raise ValueError("snapshot ref must be the frozen source ref")
