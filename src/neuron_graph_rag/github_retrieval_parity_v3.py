"""Result-free retrieval parity v3 protocol and append-only lifecycle audit."""
...
STEM = "github_retrieval_parity_v3"
PROTOCOL_ID = "github-rag-vs-ngr-retrieval-parity-v3"
...
SURFACE_AUDIT_PATH = FIXTURES / f"{STEM}.surface-audit.json"
...
def load_protocol(
    ...
    relatives = {
        ...
        "surface_audit": SURFACE_AUDIT_PATH.relative_to(ROOT),
    }
    ...
    protocol: dict[str, Any] = {
        ...
        "surface_audit": read_json(paths["surface_audit"]),
...
def validate_protocol(protocol: Mapping[str, Any]) -> None:
    ...
    for artifact in (
        ...
        _mapping(protocol, "surface_audit"),
    ):
        if artifact.get("protocol_id") != PROTOCOL_ID:
            raise ValueError("protocol_id mismatch")
    if manifest.get("issue") != 220 or manifest.get("phase") != "result-free-freeze-v3":
    ...
    expected_protocol_artifacts = {
        ...
        for name, path in {
            ...
            "audit": AUDIT_PATH,
            "surface_audit": SURFACE_AUDIT_PATH,
        }.items()
    }
    ...
    expected_source = {
        ...
        "commit": "51623e200ab6128cf59cf5b5ac7d8115c31268d6",
        "tree_sha": "034101e7ff123f91d089640e0685514a3a890159",
        "path_glob": "**/*.md",
        "document_count": 93,
        "access": "public-read-only-github-snapshot-adapter",
        "generated_by": "tools/acquire_github_markdown_snapshot_v3.py",
    }
    ...
    if (
        ...
        or corpus_mapping.get("tree_sha") != source["tree_sha"]
        ...
        raise ValueError("corpus does not match the frozen full Markdown surface")
    _validate_snapshot_hashes(corpus)

    surface_audit = _mapping(protocol, "surface_audit")
    if surface_audit != {
        "protocol_id": PROTOCOL_ID,
        "audit_type": "queryless-current-index-surface",
        "repository": corpus.repository,
        "source_commit": corpus.commit,
        "source_tree_sha": source["tree_sha"],
        "observed_from": "2026-01-22T12:27:41Z",
        "observed_through": "2026-09-07T07:15:00Z",
        ...
        "indexed_document_count": 93,
        "unique_path_count": 93,
        "duplicate_path_count": 0,
        "missing_path_count": 0,
        "out_of_surface_path_count": 0,
        "error_count": 0,
        "indexed_paths": sorted(paths),
    }:
        raise ValueError("queryless index surface audit mismatch")
    ...
    predecessor_query_hashes: set[str] = set()
    ...
    for predecessor in predecessors:
        ...
        if not isinstance(query_hashes, list) or not all(
            ...
            raise ValueError("predecessor query text hash registry mismatch")
        predecessor_query_hashes.update(query_hashes)
        gold_sources = predecessor.get("gold_source_ids")
        if not isinstance(gold_sources, list) or not all(
            isinstance(item, str) for item in gold_sources
        ):
            raise ValueError("predecessor gold source registry mismatch")
    ...
    if all_case_ids & predecessor_cases:
        raise ValueError("v3 query identity overlaps a predecessor")
    if all_query_hashes & predecessor_query_hashes:
        raise ValueError("v3 query text overlaps a predecessor")
    if set().union(*stage_identities.values()) & predecessor_gold_sources:
        raise ValueError("v3 gold source identity overlaps a predecessor")
    ...
    if new_paths & predecessor_paths:
        raise ValueError("v3 artifact registry overlaps a predecessor")
    if set(_frozen_hashes(manifest).values()) & predecessor_hashes:
        raise ValueError("v3 artifact bytes reuse a predecessor artifact")
...
def verify_protocol_commit(protocol_commit: str, protocol: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise ValueError("protocol_commit must first introduce the v3 manifest")
    ...
    if introductions.returncode != 0 or introductions.stdout.splitlines() != [
        ...
        raise ValueError("protocol_commit must uniquely introduce the v3 manifest")
    if (
...
def _run_ngr_once(
    ...
    with TemporaryDirectory(prefix="ngr-retrieval-parity-v3-") as directory:
