...
def project_relation_path(steps: list[dict[str, Any]]) -> list[dict[str, str]]:
    """Return the stable endpoint/type identity for an observed relation path."""
    return [
        {field: str(step[field]) for field in PATH_IDENTITY_FIELDS}
        for step in steps
    ]


def read_feedback_adaptation_reproduction_manifest(path: str | Path) -> dict[str, Any]:
    manifest_path = Path(path)
    manifest = _read_json(manifest_path)
    if manifest.get("schema_version") != REPRODUCTION_SCHEMA_VERSION:
        raise ValueError("Unsupported feedback adaptation reproduction schema version")
    if manifest.get("candidate_id") != "trace-credited-feedback-reproduction":
        raise ValueError("Feedback adaptation reproduction candidate is not frozen")
    ...
    for stage in ("development", "holdout"):
        ...
        for field in ("fixture", "gold", "provenance"):
            ...
            if _canonical_sha256(artifact) != split[f"{field}_sha256"]:
                raise ValueError(f"Frozen reproduction {stage} {field} hash mismatch")
    ...
    if _canonical_sha256(audit_path) != audit["artifact_sha256"]:
        raise ValueError("Frozen reproduction contamination audit hash mismatch")
    if not _read_json(audit_path).get("passed"):
        raise ValueError("Frozen reproduction contamination audit did not pass")
    return manifest


def run_feedback_adaptation_reproduction_development(path: str | Path) -> dict[str, Any]:
    manifest_path = Path(path)
    manifest = read_feedback_adaptation_reproduction_manifest(manifest_path)
...
def run_feedback_adaptation_reproduction_holdout(
    ...
    if development.get("manifest_sha256") != _canonical_sha256(manifest_path):
        raise ValueError("Development result does not match frozen reproduction manifest")
    if not development.get("gate_passed"):
        raise ValueError("Stop rule forbids opening feedback adaptation reproduction holdout")
    ...
    return {
        ...
        **result,
        "decision": {
            ...
            else "feedback_candidate_failed_frozen_holdout_gate",
        },
    }


def write_feedback_adaptation_reproduction_result(path: str | Path, result: dict[str, Any]) -> None:
...
def _evaluate_split(path: Path, manifest: dict[str, Any], stage: str) -> dict[str, Any]:
    ...
    return {
        ...
        "gate": {
            ...
            "expected_relation_endpoint_and_edge_type_match": all(
                case["path_matched"] for case in relation_treatment
            ),
            ...
            "only_credited_edges_change": treatment["feedback"]["credited_only"]
            and control["feedback"]["no_edge_mutation"],
    ...
    with NeuronGraphRAG(config=config) as engine:
        ...
        if mutate:
            receipt = engine.record_success(
                channels.relation.trace_id, [feedback["used_node_id"]], now=float(feedback["now"]) + 1.0
            )
    ...
    return {
        ...
        "feedback": {
            ...
            "credited_only": bool(credited) and all(
                (edge["source_id"], edge["target_id"], edge["edge_type"]) in credited_keys
                for edge in changed
            ),
        },
    }
...
def _validate_gold(fixture_path: Path, gold_path: Path) -> None:
    ...
    for case in cases:
        ...
        for step in case.get("expected_path", []):
            ...
            identity = tuple(str(step[field]) for field in PATH_IDENTITY_FIELDS)
