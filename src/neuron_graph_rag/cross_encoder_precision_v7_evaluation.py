...
_CONTENT_SOURCE = (
    ...
    / "containers"
    / "github_cross_encoder_precision_v7"
    / "runtime_content.py"
)
_SPEC = importlib.util.spec_from_file_location(
    "neuron_graph_rag._cross_encoder_precision_v7_frozen_base", _V3_SOURCE
)
...
_CONTENT_SPEC = importlib.util.spec_from_file_location(
    "neuron_graph_rag._cross_encoder_precision_v7_runtime_content", _CONTENT_SOURCE
)
...
STEM = "github_cross_encoder_precision_v7"
PROTOCOL_ID = "github-ngr-cross-encoder-precision-v7"
ARTIFACT_KINDS = (
    "corpus",
    "queries",
    "gold",
...
# The v3 evaluator is the literal rank-only semantic base for v4, v5, and v7.
...
def _validate_dependency_contract(protocol: Mapping[str, Any]) -> None:
    ...
    if resolver != {
        ...
        "compile_command": [
            ...
            "--output-file",
            "tests/fixtures/github_cross_encoder_precision_v7.requirements.lock",
            "tests/fixtures/github_cross_encoder_precision_v7.requirements.in",
        ],
    }:
    ...
    if _mapping(platform, "installer") != {
        ...
        "report_path": "/opt/ngr-v7/dependency-report.json",
    }:
    ...
    if _locked_versions(lock) != _locked_versions(v6_lock):
        raise ValueError("v6/v7 dependency version drift")
    ...
    expected_path = (
        "tests/fixtures/github_cross_encoder_precision_v7.expected-distributions.json"
    )
    if (
...
def _validate_container_contract(protocol: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        or build_a.get("tag") != "ngr-cross-encoder-precision-v7:freeze"
        or build_b.get("tag") != "ngr-cross-encoder-precision-v7:rebuild-check"
    ):
    ...
    for name, tag in (
        ("build_a", "ngr-cross-encoder-precision-v7:freeze"),
        ("build_b", "ngr-cross-encoder-precision-v7:rebuild-check"),
    ):
        ...
        expected_record = {
            "command": [
                ...
                "--no-cache",
                "--file",
                "containers/github_cross_encoder_precision_v7/Containerfile",
                "--tag",
                tag,
                ".",
            ],
    ...
    if run_root != {
        "path": "/opt/ngr-v7/runtime",
        "filesystem": "container",
        "exclusive_create": True,
        "host_bind_mount": False,
        "shared_windows_database": False,
        "future_observation_volume": "github-cross-encoder-precision-v7-runtime",
    }:
...
def _validate_content_equivalence(
    ...
    expected_paths = {
        "runtime_content_build_a": "tests/fixtures/github_cross_encoder_precision_v7.runtime-content.build-a.json",
        "runtime_content_build_b": "tests/fixtures/github_cross_encoder_precision_v7.runtime-content.build-b.json",
        "attestation_build_a": "tests/fixtures/github_cross_encoder_precision_v7.attestation.build-a.json",
        "attestation_build_b": "tests/fixtures/github_cross_encoder_precision_v7.attestation.build-b.json",
    }
    ...
    if outcome == PENDING_OUTCOME:
        ...
        if any(
            ...
            for key in ("fingerprint_sha256", "attestation_sha256")
        ) or any(
    ...
    if outcome == FAILURE_OUTCOME:
        failure_path = (
            "tests/evidence/github_cross_encoder_precision_v7/"
        ...
        if any(
            ...
            raise ValueError("v7 failure equivalence outcome mismatch")
        failure = _mapping(contract, "failure_evidence")
        if failure.get("path") != failure_path or _sha256(
            root / failure_path
        ) != failure.get("sha256"):
            raise ValueError("v7 failure evidence registry mismatch")
        ...
        if (
            ...
            raise ValueError("v7 failure evidence content mismatch")
        return
    if outcome != SUCCESS_OUTCOME:
        raise ValueError("unsupported v7 freeze outcome")
    ...
    if (
        ...
        raise ValueError("v7 success outcome contract mismatch")


def validate_protocol(protocol: Mapping[str, Any]) -> None:
    audit = _mapping(protocol, "result_free_audit")
    if audit.get("count_scope") != (
        "v7 registered query/model inference/observed result counts only; "
        "historical v1/v2/v3/v4/v5/v6 observations excluded"
    ):
    ...
    if outcome not in {PENDING_OUTCOME, SUCCESS_OUTCOME, FAILURE_OUTCOME}:
        raise ValueError("v7 freeze outcome mismatch")
    if audit.get("container_attempt_count_scope") != (
        "v7 one-shot build A/B and their original offline report collection only"
    ):
    ...
    if (
        ...
        raise ValueError("v7 audit outcome mismatch")
    ...
    if (
        ...
        raise ValueError("v7 output path isolation mismatch")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Audit the result-free WSLC cross-encoder precision v7 protocol"
    )
    ...
    output: dict[str, Any] = {
        ...
        "historical_v1_v2_v3_v4_v5_v6_observations_included": False,
