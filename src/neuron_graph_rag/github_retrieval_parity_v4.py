"""Result-free retrieval parity v4 protocol and append-only lifecycle audit."""
...
STEM = "github_retrieval_parity_v4"
PROTOCOL_ID = "github-rag-vs-ngr-retrieval-parity-v4"
...
def validate_protocol(protocol: Mapping[str, Any]) -> None:
    ...
    if manifest.get("issue") != 222 or manifest.get("phase") != "result-free-freeze-v4":
    ...
    for predecessor in predecessors:
        ...
        if predecessor.get("name") == "retrieval-parity-v3":
            if predecessor.get("registered_query_execution_count") != 0:
                raise ValueError("v3 query derivation requires literal zero-query evidence")
            v3_audit = read_json(
                root / "tests/fixtures/github_retrieval_parity_v3.result-free-audit.json"
            )
            if (
                v3_audit.get("development_query_execution_count") != 0
                or v3_audit.get("holdout_query_execution_count") != 0
                or any(
                    v3_audit.get(key) != 0
                    for key in (
                ...
                raise ValueError("v3 registered observation evidence is not result-free")
            v3_query_hashes = set(query_hashes)
        else:
            older_query_hashes.update(query_hashes)
    if all_case_ids & predecessor_cases:
        raise ValueError("v4 case identity overlaps a predecessor")
    if v3_query_hashes is None or all_query_hashes != v3_query_hashes:
        raise ValueError("v4 query texts must derive exactly from unobserved v3 queries")
    if all_query_hashes & older_query_hashes:
        raise ValueError("v4 query text overlaps an observed predecessor")
    if set().union(*stage_identities.values()) & predecessor_gold_sources:
        raise ValueError("v4 gold source identity overlaps a predecessor")
    ...
    if new_paths & predecessor_paths:
        raise ValueError("v4 artifact registry overlaps a predecessor")
    ...
    expected_audit = {
        ...
        "v3_query_text_derivation_count": 10,
...
def verify_protocol_commit(protocol_commit: str, protocol: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise ValueError("protocol_commit must first introduce the v4 manifest")
    ...
    if introductions.returncode != 0 or introductions.stdout.splitlines() != [
        ...
        raise ValueError("protocol_commit must uniquely introduce the v4 manifest")
    if (
...
def _run_ngr_once(
    ...
    with TemporaryDirectory(prefix="ngr-retrieval-parity-v4-") as directory:
