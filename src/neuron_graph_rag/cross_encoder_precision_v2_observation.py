...
from .cross_encoder_precision_v2_evaluation import (
    PROTOCOL_ID,
    ROOT,
    archive_stage,
...
PROTOCOL_COMMIT = "36c17aac3b49587c97d96bac51db668bf834177b"
SOURCE_COMMIT = "c32b3049fd3daaa2190faf5e3e85955a195ee88c"
OBSERVATION_NOW = 1_700_000_000.0
BATCH_SIZE = 8
EVIDENCE = Path("tests/evidence/github_cross_encoder_precision_v2")
PREFLIGHT_FILES = (
    "preflight.json",
...
def default_external_root() -> Path:
    return ROOT.parent.parent / "experiments" / "github_cross_encoder_precision_v2"
...
def preflight(
    ...
    lock = root / "tests/fixtures/github_cross_encoder_precision_v2.requirements.lock"
    _run_logged(
    ...
    _run_logged(
        [
            str(python),
            "-m",
            "neuron_graph_rag.cross_encoder_precision_v2_observation",
            "model-verify",
            "--cache",
            str(cache),
            "--output",
    ...
    for arguments in (
        [str(python), "-m", "unittest", "tests.test_cross_encoder_precision_v2"],
        [
            str(python),
            "-m",
            "unittest",
            "tests.test_cross_encoder_precision_v2_observation",
        ],
        [
            str(python),
            "-m",
            "neuron_graph_rag.cross_encoder_precision_v2_evaluation",
            "audit",
        ],
        [
            str(python),
...
def verify_preflight(
    ...
    if (
        ...
        or report.get("cache_reused_as_verified_bytes_only") is not True
