...
STEM = "baseline_aware_soft_start_snapshot_v2"
...
def derive_q3_first_mutation_event(quorum: int, initial_evidence_count: int) -> int:
    if quorum < 1 or initial_evidence_count < 0:
        raise ValueError("quorum must be positive and initial evidence non-negative")
    return max(1, quorum - initial_evidence_count)
...
def _select_trace(engine: NeuronGraphRAG, case: Mapping[str, Any], now: float) -> str:
    ...
    if case["search_surface"] == "relation" and case["case_role"] in {"confirmed", "corrected"}:
        ...
        selected_edges = () if selected is None else tuple(
            (step["source_id"], step["target_id"], step["edge_type"])
            for step in selected["steps"]
        )
        if selected_edges != (expected,):
            raise RuntimeError("registered credited edge is not the selected relation path")
...
def evaluate_gates(
    ...
    for arm in arms.values():
        for observed_case in arm["cases"]:
            case = registered[str(observed_case["case_id"])]
            if case["case_role"] not in {"confirmed", "corrected"}:
                continue
            edge = _edge_at(
                _checkpoint_by_name(observed_case, "baseline"),
                _edge_key(case["credited_edge"]),
            )
            baseline_matches = baseline_matches and all(
                edge[field] == case["registered_initial_state"][field]
                for field in (
                    "weight",
                    "reinforced_count",
                    "evidence_count",
    ...
    protocol_integrity = bool(preflight["passed"]) and baseline_matches and all(
    ...
    tolerance = float(thresholds["numeric_tolerance"])
    soft_schedule = (
        provisional > 0.0
        and first_total <= confirmed_reference + tolerance
        and abs(first_total - confirmed_reference) <= tolerance
        and [item["multiplier"] for item in later] == [0.5, 0.25, 0.125]
        and all(
            current["actual_delta"] <= previous["actual_delta"]
    ...
    baseline_q3_boundary = True
    event_budget = int(thresholds["event_budget"])
    for role in ("confirmed", "corrected"):
        q3_case = _case_by_role(used, role)
        case = registered[str(q3_case["case_id"])]
        edge_key = _edge_key(case["credited_edge"])
        first = int(case["expected_q3_first_mutation_event"])
        deltas = [_delta(q3_case, f"used_{event}", edge_key) for event in range(1, event_budget + 1)]
        evidence = [
            int(_edge_at(_checkpoint_by_name(q3_case, f"used_{event}"), edge_key)["evidence_count"])
            for event in range(1, event_budget + 1)
        ]
        initial_evidence = int(case["registered_initial_state"]["evidence_count"])
        baseline_q3_boundary = baseline_q3_boundary and (
            all(delta == 0.0 for delta in deltas[: first - 1])
            and deltas[first - 1] > 0.0
            and all(current > previous for previous, current in zip(deltas[first - 1 :], deltas[first:]))
            and evidence == [initial_evidence + event for event in range(1, event_budget + 1)]
        )

    confirmed_receipts = confirmed_only_case["receipts"]
    confirmations = [
        receipt["outcome"]["confirmations"][0]
        for receipt in confirmed_receipts
        if receipt["outcome"]["confirmations"]
    ]
    confirmed_used_nonmutating = (
        _delta(confirmed_only_case, "used_1", confirmed_key) == 0.0
        and all(
            _delta(confirmed_only_case, f"used_{event}", confirmed_key)
            == _delta(confirmed_only_case, f"outcome_{event - 1}", confirmed_key)
            for event in range(2, event_budget + 1)
        )
    )
    confirmed_boundary = (
        confirmed_used_nonmutating
        and [item["multiplier"] for item in confirmations]
        == [1.0, 0.5, 0.25, 0.125]
        and all(item["actual_delta"] > 0.0 for item in confirmations)
    )
    ...
    soft_final = _checkpoint_by_name(confirmed_case, "outcome_4")["metrics"]["target_mrr"]
    used_final = _checkpoint_by_name(used_case, "outcome_4")["metrics"]["target_mrr"]
    latency_quality = (
        provisional > 0.0
    ...
    corrected_cost = _delta(corrected, "outcome_4", corrected_key)
    negative_cost = (
        corrected_cost > 0.0
        and corrected_cost
        <= corrected_base * float(thresholds["negative_provisional_ratio_max"])
        + tolerance
    ...
    for arm in arms.values():
        for case in arm["cases"]:
            ...
            final = _checkpoint_by_name(case, "outcome_4")
    ...
    return {
        ...
        "baseline-aware-q3-boundary": baseline_q3_boundary,
        "confirmed-only-boundary": confirmed_boundary,
...
def _preflight(
    ...
    with sqlite3.connect(snapshot.as_uri() + "?mode=ro", uri=True) as connection:
        ...
        registered_state_matches = True
        derivation_matches = True
        capacity_available = True
        event_budget = int(artifacts["schedule"]["event_budget"])
        quorum = 3
        for case in cases:
            if case["case_role"] not in {"confirmed", "corrected"}:
                continue
            identity = tuple(str(case["credited_edge"][field]) for field in EDGE_FIELDS)
            ...
            actual = {
                "weight": None if edge is None else float(edge[0]),
                "reinforced_count": None if edge is None else int(edge[1]),
                "evidence_count": evidence_count,
                "confirmation_count": 0 if confirmed is None else int(confirmed[0]),
            }
            expected_event = derive_q3_first_mutation_event(quorum, evidence_count)
            registered_state_matches = registered_state_matches and (
                actual == case["registered_initial_state"]
            )
            derivation_matches = derivation_matches and (
                expected_event == case["expected_q3_first_mutation_event"]
            )
            capacity_available = capacity_available and expected_event < event_budget
    registered_nodes = {
        str(case[field])
        for case in cases
    ...
    checks = {
        ...
        "registered_initial_state": registered_state_matches,
        "baseline_derivation": derivation_matches,
        "event_budget_capacity": capacity_available,
        "v1_development_edge_isolation": stage != "development"
        or registered_edges.isdisjoint(
            set(manifest["excluded_v1_development_credited_edges"])
        ),
...
def run_registered_stage(stage: str, snapshot: Path) -> Path:
    ...
    except Exception:  # noqa: BLE001 - a one-time protocol failure is evidence
