...
RUN_ROOT = Path("/opt/ngr-v7/runtime")
EXPECTED_REGISTRY = Path("/opt/ngr-v7/expected-distributions.json")
...
def _load_expected() -> tuple[dict[str, str], str]:
    ...
    if (
        ...
        or value["protocol_id"] != "github-ngr-cross-encoder-precision-v7"
    ):
