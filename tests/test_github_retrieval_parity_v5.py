...
from neuron_graph_rag import github_retrieval_parity_v5 as parity
...
class GitHubRetrievalParityV5Tests(unittest.TestCase):
    ...
    def test_materialized_corpus_is_exact_and_path_isolated(self) -> None:
        ...
        self.assertEqual(
            parity.CORPUS_PATH,
            ROOT / "tests/fixtures/github_retrieval_parity_v4.corpus.json",
        )
        self.assertFalse(
            (ROOT / "tests/fixtures/github_retrieval_parity_v5.corpus.json").exists()
        )
    ...
    def test_split_and_predecessor_identity_boundaries(self) -> None:
        ...
        v4_query_hashes = {
            item
            for predecessor in protocol["manifest"]["predecessors"]
            if predecessor["name"] == "retrieval-parity-v4"
            for item in predecessor["query_text_sha256"]
        }
        self.assertEqual(query_hashes, v4_query_hashes)
        ...
        self.assertEqual(
            identities("development") | identities("holdout"),
            predecessor_gold_sources,
        )
    ...
    def test_frozen_predecessor_and_v5_artifact_hashes_match(self) -> None:
        protocol = parity.load_protocol()
        self.assertEqual(
        ...
        with self._protocol_root() as directory:
            ...
            with self.assertRaisesRegex(
                ValueError, "predecessor artifact drifted|frozen artifact hash mismatch"
            ):
    ...
    def test_tampered_result_and_split_overlap_are_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "semantics drifted from v4"):
    ...
    def test_runner_audit_executes_no_registered_query(self) -> None:
        result = subprocess.run(
            [sys.executable, "tools/run_github_retrieval_parity_v5.py", "--audit"],
            cwd=ROOT,
            check=True,
            capture_output=True,
            text=True,
    ...
    def test_runner_import_smoke_is_clean_without_ambient_pythonpath(self) -> None:
        protocol = parity.load_protocol()
        registry_paths = [
            ROOT / relative
        ...
        self.assertFalse(any(path.exists() for path in registry_paths))
        ...
        with tempfile.TemporaryDirectory() as home:
            ...
            result = subprocess.run(
                [
                    sys.executable,
                    "-I",
                    "tools/run_github_retrieval_parity_v5.py",
                    "--import-smoke",
                ],
                cwd=ROOT,
                env=environment,
            ...
            self.assertFalse((Path(home) / ".ngrdb" / "knowledge.db").exists())

        payload = json.loads(result.stdout)
        self.assertEqual(payload["status"], "import-safe")
        self.assertTrue(payload["repository_root_resolved"])
        self.assertTrue(payload["source_root_resolved"])
        self.assertEqual(payload["github_rag_request_count"], 0)
        self.assertEqual(payload["ngr_search_count"], 0)
        self.assertEqual(payload["sqlite_open_count"], 0)
        self.assertEqual(payload["artifact_create_count"], 0)
        self.assertFalse(any(path.exists() for path in registry_paths))

    def test_import_smoke_does_not_reach_effect_surfaces(self) -> None:
        output = io.StringIO()
        runner = ROOT / "tools/run_github_retrieval_parity_v5.py"
        argv = [str(runner), "--import-smoke"]
        blocked = AssertionError("import smoke reached a forbidden effect surface")
        with (
            mock.patch.object(sys, "argv", argv),
            mock.patch.object(sqlite3, "connect", side_effect=blocked),
            mock.patch.object(socket.socket, "connect", side_effect=blocked),
            mock.patch.object(parity.NeuronGraphRAG, "search", side_effect=blocked),
            mock.patch.object(parity, "write_json_exclusive", side_effect=blocked),
            mock.patch.object(parity, "register_preflight", side_effect=blocked),
            mock.patch.object(parity, "register_capture", side_effect=blocked),
            mock.patch.object(parity, "run_registered_stage", side_effect=blocked),
            contextlib.redirect_stdout(output),
        ):
            runpy.run_path(str(runner), run_name="__main__")
        self.assertEqual(json.loads(output.getvalue())["status"], "import-safe")
