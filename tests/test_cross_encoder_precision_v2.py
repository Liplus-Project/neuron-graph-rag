...
from neuron_graph_rag import cross_encoder_precision_evaluation as v1_evaluation
from neuron_graph_rag import cross_encoder_precision_v2_evaluation as evaluation

ROOT = Path(__file__).resolve().parents[1]


class CrossEncoderPrecisionV2FreezeTest(unittest.TestCase):
    ...
    def test_all_empty_candidates_round_trip_as_failed_without_selection(self) -> None:
        protocol = evaluation.load_protocol()
        with tempfile.TemporaryDirectory() as directory:
            synthetic = dict(protocol)
            synthetic["root"] = Path(directory)
        ...
        self._empty_model_cases(models, range(8))
        ...
        self.assertTrue(
            all(
                not candidate["gates"][6]["passed"]
                for candidate in result["candidates"]
            )
        )

    def test_partial_negative_and_baseline_missing_empty_cases_round_trip(self) -> None:
        protocol = evaluation.load_protocol()
        scenarios = (
            ("partial", (0,), None),
            ("negative-only", (6, 7), None),
            ("baseline-expected-missing", (0,), (0, "expected_paths")),
            ("baseline-clean-negative", (6,), (6, "forbidden_paths")),
        )
        for name, empty_indices, baseline_change in scenarios:
            with self.subTest(name=name), tempfile.TemporaryDirectory() as directory:
                ...
                baseline = evaluation._raw_baseline(valid["baseline"])
                models = [evaluation._raw_model(row) for row in valid["models"]]
                self._empty_model_cases(models, empty_indices)
                if baseline_change:
                    case_index, gold_key = baseline_change
                    target = protocol["gold"]["stages"]["development"][case_index][
                        gold_key
                    ][0]
                    for hits in (
                        baseline["cases"][case_index]["ranked_hits"],
                        *[
                            model["cases"][case_index]["ranked_hits"]
                            for model in models
                        ],
                    ):
                        hit = next(row for row in hits if row["source_path"] == target)
                        hits.remove(hit)
                        hits.insert(5, hit)
                ...
                evaluation.verify_result_payload(protocol, "development", result, claim)
                for candidate in result["candidates"]:
                    for case_index in empty_indices:
                        self.assertEqual(
                            candidate["cases"][case_index]["returned_source_paths"], []
                        )
    ...
    def test_v1_v2_semantic_parity_and_v1_byte_immutability(self) -> None:
        v2 = evaluation.load_protocol()
        v1 = v1_evaluation.load_protocol()
        for key in (
            "corpus",
            "queries",
            "gold",
            ...
            left = copy.deepcopy(v1[key])
            right = copy.deepcopy(v2[key])
            left["protocol_id"] = evaluation.PROTOCOL_ID
            self.assertEqual(left, right, key)
        self.assertEqual(v1["requirements_lock"], v2["requirements_lock"])
        self.assertEqual(v1_evaluation.CANDIDATE_IDS, evaluation.CANDIDATE_IDS)
        self.assertEqual(v1_evaluation.GATE_IDS, evaluation.GATE_IDS)
        for function in (
            "project_passages",
            "sigmoid",
            "_derive_candidate",
            ...
            self.assertEqual(
                inspect.getsource(getattr(v1_evaluation, function)),
                inspect.getsource(getattr(evaluation, function)),
                function,
            )

        registry = v2["manifest"]["v1_immutable_sha256"]
        expected = {
    ...
    def test_only_freeze_manifest_introduction_commit_is_accepted(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            manifest = (
                root / "tests/fixtures/github_cross_encoder_precision_v2.manifest.json"
            )
    ...
    @staticmethod
    def _empty_model_cases(
        models: list[dict[str, object]], case_indices: object
    ) -> None:
        indices = set(case_indices)
        for model in models:
            for case_index, case in enumerate(model["cases"]):
