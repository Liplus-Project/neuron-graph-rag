...
from neuron_graph_rag import (
    cross_encoder_precision_v12_performance_observation as observation,
)


class CrossEncoderPrecisionV12PerformanceObservationTest(unittest.TestCase):
    def test_identity_workers_and_paths_are_exact(self) -> None:
        self.assertEqual(
            observation.FREEZE_COMMIT,
            "39f2cebc6b3b43ac1060a2ce519e8906fa598f57",
        )
        self.assertEqual(observation.WSLC_VERSION, "2.9.4.0")
        self.assertEqual(
            observation.VOLUME, "github-cross-encoder-precision-v12-runtime"
        )
        self.assertEqual(
    ...
    def test_container_command_is_offline_and_mounts_only_v12_runtime(self) -> None:
        command = observation._container_command(
            "worker",
            "--stage",
            "development",
            name="ngr-v12-development-baseline-primary",
        )
        rendered = "\n".join(command)
        ...
        self.assertEqual(
            ...
            "github-cross-encoder-precision-v12-runtime:/opt/ngr-v12/runtime",
        )
        for forbidden in (
            observation.V11_ROOT_FREEZE_VOLUME,
    ...
    def test_source_initialization_leaves_model_cache_and_old_root_absent(self) -> None:
        ...
        self.assertIn("test ! -e '/opt/ngr-v12/runtime/model-cache'", script)
        self.assertIn("test ! -e '/opt/ngr-v8/runtime'", script)
        self.assertIn("mkdir '/opt/ngr-v12/runtime/source'", script)
        self.assertNotIn("mkdir -p /opt/ngr-v12/runtime/model-cache", script)
        for path in ("databases", "runs", "archive", "transport"):
            self.assertIn(f"/opt/ngr-v12/runtime/{path}", script)
    ...
    def test_model_copy_uses_frozen_exclusive_verifier(self) -> None:
        with patch.object(
            ...
            result = observation._container_model_copy(
                "/input/models",
                "/opt/ngr-v12/runtime/model-cache",
                "/opt/ngr-v12/runtime/model-verification.json",
            )
        ...
        self.assertEqual(
            ...
            "/opt/ngr-v12/runtime/source/tests/fixtures/"
            "github_cross_encoder_precision_v8.models.json",
        )

    def test_configure_uses_v11_binder_for_actual_v12_roots(self) -> None:
        with (
            patch.object(Path, "exists", return_value=False),
            patch.object(
                observation.root_freeze,
                "bind_frozen_harness_root",
                return_value={"wrapper_base_distinct": True},
            ) as binder,
        ):
        ...
        self.assertEqual(
            binder.call_args.kwargs["root"], Path("/opt/ngr-v12/runtime")
        )
        self.assertEqual(
            binder.call_args.kwargs["source"],
            Path("/opt/ngr-v12/runtime/source"),
        )
        self.assertEqual(
            binder.call_args.kwargs["protocol_source"],
            Path("/opt/ngr-v12/runtime/frozen-source"),
        )
        self.assertEqual(
            binder.call_args.kwargs["evidence"],
            Path(
                "/opt/ngr-v12/runtime/source/tests/evidence/"
                "github_cross_encoder_precision_v12_observation"
            ),
        )

    def test_v12_scope_restores_v10_and_nested_lifecycle_defaults(self) -> None:
        original = {
    ...
    def test_platform_evidence_names_all_legacy_boundaries(self) -> None:
        ...
        self.assertEqual(
            report["v11_root_freeze_volume"], observation.V11_ROOT_FREEZE_VOLUME
        )
        for key in (
    ...
    def test_manifest_freezes_v11_closure_and_v8_image_contract(self) -> None:
        ...
        self.assertEqual(
            contract["v11_root_binding_sha256"],
            observation.V11_ROOT_BINDING_SHA256,
        )
    ...
    def test_worker_commands_fix_six_fresh_processes_and_databases(self) -> None:
        ...
        self.assertTrue(all(name.startswith("ngr-v12-") for name in names))
        self.assertTrue(
            all(
                path.startswith("/opt/ngr-v12/runtime/databases/development/")
