...
from neuron_graph_rag import cross_encoder_precision_observation as observation
...
class CrossEncoderPrecisionObservationTest(unittest.TestCase):
    def test_observation_identity_and_offline_environment_are_fixed(self) -> None:
        self.assertEqual(
            ...
            "b681b2a8073e13dfc940b6f3e0c55f86556a21c0",
        )
        self.assertEqual(
    ...
    def test_verify_preflight_rejects_tampered_binding(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            lock = fixture / "github_cross_encoder_precision_v1.requirements.lock"
            ...
            with (
                ...
                observation.verify_preflight(root, external)
    ...
    def test_failed_development_never_starts_holdout_and_records_failure(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                ...
                observation.run_conditional(root, root / "external")
            self.assertEqual(
                run.call_args_list, [call("development", root, root / "external", [])]
            )
            failure = json.loads(
    ...
    def test_holdout_runs_only_after_development_passes(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                ...
                result = observation.run_conditional(root, root / "external")
            self.assertEqual(
