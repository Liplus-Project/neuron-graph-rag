...
from neuron_graph_rag import github_retrieval_parity_v2 as parity
...
class GitHubRetrievalParityV2Tests(unittest.TestCase):
    ...
    def test_source_snapshot_is_exact_pinned_docs_surface(self) -> None:
        protocol = parity.load_protocol()
        corpus = protocol["corpus"]
        self.assertEqual(corpus.repository, "Liplus-Project/liplus-language")
        self.assertEqual(corpus.commit, "7a2d1a3a9e6fb821c836333900a9f1d145bd1832")
        self.assertEqual(len(corpus.documents), 20)
        for document in corpus.documents:
            self.assertRegex(document.path, r"^docs/[^/]+\.md$")
    ...
    def test_runner_audit_executes_no_registered_query(self) -> None:
        result = subprocess.run(
            [sys.executable, "tools/run_github_retrieval_parity_v2.py", "--audit"],
            cwd=ROOT,
            check=True,
            capture_output=True,
            text=True,
