...
GitHub の最小 read-only adapter 候補は、coreへGitHub clientを持ち込まず、固定snapshotをlocal indexへ接続する形で検証しています。[github-rag-mcp replacement compatibility spike](docs/github-rag-mcp-replacement-compatibility.md) は、public repository一つの取得、保存済み `search_issues` capture との比較、one-document update follow-upだけを扱います。capture がない固定snapshotの実行は adapter pipeline validation に限り、production serviceやMCP replacementを主張するものではありません。
